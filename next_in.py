...
class Next_In:
    def __init__(self, next_in_path, iteration_path, ses_version):
        ...
        if self.ses_version in ["SI_TO_IP", "IP_TO_SI"]:
    ...
    def create_iterations(self, iteration_worksheet_name):
        self.create_map_df()
        self.read_iteration(iteration_worksheet_name)
    ...
    def rows_to_input_df(
        ...
        if column_to_text == 0:
            if number_of_rows == 1:
                ...
                self.input_df = pd.concat(
                    [self.input_df, df], ignore_index=True, axis=0
                )
        ...
        # If there is text in the input, use this function
            ...
            if column_to_text == 1:  # Text before number values
                row_list_collection = worksheet_df.iloc[
                    start_row:end_row, start_column + 1 : end_column
                ].values.tolist()
                text_input_list.extend(row_list_collection[0])
                row_list = text_input_list
            else:  # Assume text is at end of line
                row_list_collection = worksheet_df.iloc[
                    start_row:end_row, start_column : start_column + column_to_text - 1
                ].values.tolist()
            ...
            self.input_df.loc[
                len(self.input_df)
            ] = row_list  # Add to last line of inputfile dataframe
        ...
        for next_in_row in range(start_row, end_row):
            ...
            for next_in_column in range(start_column, end_column):
                ...
                if (
                    entry_number == entry_to_text
                ):  # If the entry is text, enter how many entries become text
    ...
    def convert_input_units(self, row_list, SI_2_IP_Units):
        ...
        for i in range(len(row_list)):
            ...
            if isinstance(value, str):
                ...
                elif self.ses_version == "IP_TO_SI":
                    converted_row_list.append(
    ...
    def columns_to_row_input(
        ...
        if row_to_text == 0:
            row_list = (
                worksheet_df.iloc[start_row:end_row, start_column]
                .transpose()
                .values.tolist()
            )
        elif row_to_text > 0:
            ...
            if row_to_text == 1:  # Text before number values
                row_list_collection = (
                    worksheet_df.iloc[start_row + 1 : end_row, start_column]
                    .transpose()
                    .values.tolist()
                )
                ...
                row_list_collection = (
                    worksheet_df.iloc[start_row : end_row - 1, start_column]
                    .transpose()
                    .values.tolist()
                )
    ...
    def create_map_df(self):
        ...
        self.map_df.set_index(
            [column_names[0], column_names[1], column_names[2]], inplace=True
        )
    ...
    def form_01(self):
        ...
        row_is_blank = self.input_df.apply(
            lambda row: row.str.count(" ").sum() == 80, axis=1
        )
        found_text = (
            False  # Initialy, assume the row is blank and text will not be found
        )
    ...
    def form_02(self):
        ...
        num_line_sec = (
            self.sections - self.vent_sections
        )  # Number of line sections for tunnels
        start_row = 4
        start_column = 1
        columns_to_read = 5
        self.rows_to_input_df(
            worksheet_df, start_row, start_column, columns_to_read, num_line_sec
        )
        # Form 2B
    ...
    def form_03(self):
        ...
        while row < len(worksheet_df):
            ...
            if not pd.isna(
                worksheet_df.iloc[row, column]
            ):  # If this is not a NaN value
                # Form 3A Part 1
                columns_to_read = 3
                self.rows_to_input_df(
                    worksheet_df,
                ...
                if not pd.isna(F08A_df.iloc[row_8A, col_8A]):
                    ...
                    if groups_of_trains > 1:
                        ...
                        self.rows_to_input_df(
                            ...
                            col_8B,
                            columns_to_read,
                            number_of_rows=groups_of_trains - 1,
                        )
                    ...
                    if self.train_performance_option in [2, 3]:
                        ...
                        self.rows_to_input_df(
                            worksheet_df, row_first, col_8E + 4, columns_to_read
                        )
                        speed_time_points = int(
                            worksheet_df.iloc[row_first, col_8E + 4]
                        )
                    ...
                    self.rows_to_input_df(
                        ...
                        col_8F,
                        columns_to_read,
                        number_of_rows=number_of_sections,
                    )
                    col_8F = col_8F + 3
                row_8A += 1
            ...
            if not pd.isna(worksheet_df_f11A.iloc[row_F11A, col_F11A]):
                ...
                if self.environment_control_zones > 1:
                    while read_sections < number_sections:
                        ...
                        self.columns_to_row_input(
                            worksheet_df_f11B, row_F11B, col_F11B, i
                        )
            ...
            while row_14AB < len(worksheet_df):
                ...
                if not pd.isna(worksheet_df.iloc[row_14AB, col_14AB]):
                    ...
                    if not pd.isna(worksheet_df2.iloc[row_14C, col_14C]):
                        ...
                        if (
                            columns_to_read < 8
                        ):  # If there are less than 8 entries, add blanks
                            for i in range(columns_to_read, 8):
                                df.insert(i, i, np.nan)
    ...
    def save_unformatted_input_file(self):
        input_string = self.input_df.to_string(
            index=False, header=False, col_space=10, justify="left", na_rep=""
        )
        input_path_df_to_text = self.save_path.with_suffix(".txt")
        with open(input_path_df_to_text, "w") as f:
            f.write(input_string)

    def read_iteration(self, iteration_worksheet_name):
